...
#define RE_huart    huart_Referee
#define RE_UART		USART3
#define RE_dma_rx   hdma_usart3_rx
#define RE_dma_tx   hdma_usart3_tx
...
uint8_t canTX_referee(int8_t robot_id, int16_t heat0, int16_t heat0_limit,int16_t bullet_speed, uint8_t bullet_speedlimit);


void RefereeTask(void const * argument) 
{       
	referee_usart_init();//裁判读包串口初始化
...
//	ui_renew(); 
	...
	while(1)
	{
		status_data.ui_renew_last = status_data.ui_renew;
		status_data.ui_renew = hCtrl.UIreflashFLag;
...
uint8_t canTX_referee_message(int8_t robot_id, int16_t heat0, int16_t heat0_limit,int16_t bullet_speed, uint8_t bullet_speedlimit)
{
